...
class VGCOCOIntersectionDataset():
    ...
    def __init__(self, opt, split, year):
        ...
        self.coco_dataset = COCODataset(opt, split, year)
    ...
    def dump_gold_scene_parsed_data(self):    
        ...
        for d in json.load(open(self.vg_relationships_data_file)):
            ...
            for relationship in d['relationships']:
                ...
                if id not in gold_scene_parsed_data[coco_id]['relationship_descriptors'][bbox_str]['synset_pairs']:
                     gold_scene_parsed_data[coco_id]['relationship_descriptors'][bbox_str]['synset_pairs'][id] = []
                gold_scene_parsed_data[coco_id]['relationship_descriptors'][bbox_str]['synset_pairs'][id].append(relationship)        
    ...
    def load_vqa_data(self):
        ...
        for d in vqa_annotation:
            ...
            #if self.opt.bbox_detection_type=='gold' or self.opt.object_detection_type=='gold' or self.opt.attribute_detection_type=='gold':
            #    bbox_data = self.get_bbox_data(image_id)
            #    data['bbox'] = bbox_data
            self.vqa_data[image_id].append(data)    
            print ('image id ', image_id)
            self.data_index[i] = (image_id, question_index)
            i+=1 
            if len(self.data_index)%100==0 and len(self.data_index)>0:
                break
